...
export class ListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  initDataTable() {
    ...
    this.dtOptions = {
      ...
      ajax: (dtParams: any, callback) => {
        ...
        that.orderService.ordersGet(query).subscribe((resp: HttpCollectionResponse) => {
          that.orders = resp.data.docs;
          callback({
  ...
  refreshCategories() {
    ...
    this.orderService.ordersGet(query).subscribe((resp: HttpCollectionResponse) => {
      $('.dataTables_processing').hide();
      this.orders = resp.data.docs;
      this.rerender();
    });
  }

  doAction(action: string, order: any) {
    const id = order.order_no;
    switch (action) {
      case 'view':
